...
function QuizComponent() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8081/getQuestions.php?topic=${topicName}`)
      .then((res) => {
  ...
  const submitFinalResults = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8081/SubmitResult.php", {
        method: "POST",
      ...
      {currentIndex === questions.length - 1 &&
        userAnswers.length === questions.length && (
          <button
            ...
            style={{
              background: isSubmitting ? "#999" : "#00cc99",
              color: "white",
